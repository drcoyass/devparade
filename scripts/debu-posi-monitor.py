#!/usr/bin/env python3
"""
DebuPosi Monitor - ネガティブなデブ発言を自動検知して自動返信
...
RESPONSES = {
    "デブ": [
        "デブ？ ありがとう！総体重570kg超バンドDEV PARADEから言わせてもらうと、それは最高の褒め言葉🍖 #デブポジ",
        "「デブ」= Delightful Excellent Beautiful。知らなかった？🍖 by DEV PARADE #デブポジ",
        "デブは才能。俺たち5人で570kg、全員メジャーデビュー済み。才能に体重は関係ない🍖 #デブポジ",
        "デブをネガティブに捉えないで。俺たちバンド名にデブ入れて誇ってるから🍖 @dev_parade #デブポジ",
        "デブって呼ばれた？ おめでとう、DEV PARADEの入団資格満たしてるぞ（条件:90kg以上）🍖 #デブポジ",
    ],
    "太った": [
        "太った？ それは「成長した」って言うんだよ。細胞レベルで進化してる。おめでとう🎉🍖 #デブポジ",
        "体重が増えた = 存在感が増した。DEV PARADEが保証する🍖 @dev_parade #デブポジ",
        "太った分だけ地球がお前を離したくないってこと。モテてるぜ、地球に🌍🍖 #デブポジ",
        "体重増えた？ それは「影響力」のメーター。デカい奴はデカいことができる🍖 #デブポジ",
    ],
    "痩せ": [
        "痩せる必要？ ないね。俺たち570kgで武道館目指してる。痩せたら夢が軽くなる💪🍖 #デブポジ",
        "「痩せたい」→「もっと愛されたい」に変換。大丈夫、そのままで最高🍖 @dev_parade #デブポジ",
        "痩せなくていい。むしろDEV PARADE入らない？条件は90kg以上🍖 #デブポジ",
        "ダイエットの語源は「生き方」。今の生き方、最高じゃん。変えなくていい🍖 #デブポジ",
    ],
    "generic": [
        "大丈夫、お前は最高だ。570kg超のバンドが言ってるんだから間違いない🍖 @dev_parade #デブポジ",
        "デブは個性。個性は武器。武器は磨け🍖 by DEV PARADE #デブポジ",
        "この世に無駄な脂肪なんてない。全部お前という作品の一部だ🍖 @dev_parade #デブポジ",
        "どんな悩みも、焼肉食ったら解決する。解決しなくても美味い。それでいい🍖 #デブポジ",
    ],
}
...
def generate_issue(tweets, auto_mode):
    ...
    lines.append("*Powered by DEV PARADE デブポジBot 🍖*")
